...
if (count($dataHutang) > 0) {
    ...
    $idHutangList = array_map("intval", $idHutangList);
    $placeholders = implode(",", array_fill(0, count($idHutangList), "?"));
    $types = str_repeat("i", count($idHutangList));
    ...
    $queryDetail->bind_param($types, ...$idHutangList);
    $queryDetail->execute();
    $dataDetail = $queryDetail->get_result()->fetch_all(MYSQLI_ASSOC);
...
<?php render_page_start('Laporan Hutang', 'hutang', ['assets/css/hutang.css']); ?>
